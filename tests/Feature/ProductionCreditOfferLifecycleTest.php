...
class ProductionCreditOfferLifecycleTest extends TestCase
{
    ...
    public function test_operations_can_approve_referred_decision_and_generate_immutable_offer(): void
    {
        ...
        $this->postJson("/api/admin/credit-decisions/{$decision->id}/approve", [
            ...
            ->assertOk()
            ->assertJsonPath('data.decision.status', CreditDecision::STATUS_APPROVED)
    ...
    {
        [$customer, $operations, $application, $decision] = $this->approvedApplication();
        Sanctum::actingAs($operations);
        ...
        $this->postJson("/api/credit/offers/{$offerId}/accept", [
            ...
            ->assertStatus(409)
            ->assertJsonPath('errors.disclosure_hash.0', 'DISCLOSURE_HASH_MISMATCH');
        ...
        $transaction->update([
            ...
            'provider_reference' => 'cpay-success-001',
        ]);
        ...
        $scheduledTotal = (int) $this->app['db']->table('credit_repayment_schedule_items')
            ->where('loan_id', $loan->id)
            ->sum('total_due_minor');
        $this->assertSame(165000, $scheduledTotal);
        $this->assertDatabaseHas('audit_logs', [
            'event' => 'credit.disbursement.fulfilled',
            'subject_id' => $loan->id,
        ]);
    }
