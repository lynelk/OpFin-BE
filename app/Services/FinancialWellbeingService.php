...
class FinancialWellbeingService
{
    ...
    public function compass(User $user, string $currency = 'UGX', ?CarbonImmutable $asOf = null): array
    {
        ...
        $accounts = $this->scope(DB::table('financial_accounts'), $user)
            ->where('active', true)
        ...
        $committedMinor = (int) collect($calendar)
            ->where('direction', 'expense')
            ->whereIn('certainty', ['confirmed', 'scheduled'])
            ->sum('amount_minor');
    ...
    public function budgets(
        ...
        return $budgets->map(function ($budget) use ($user, $from, $to, $currency) {
            ...
            return [
                ...
                'actual_minor' => $actual,
                'remaining_minor' => max(0, $limit - $actual),
                'utilization_percent' => $percent,
                'alert_threshold_percent' => (int) $budget->alert_threshold_percent,
                'status' => $actual > $limit
                    ? 'over_budget'
                    : ($percent >= (int) $budget->alert_threshold_percent ? 'attention' : 'on_track'),
                'currency' => $currency,
            ];
        })->values()->all();
    }
    ...
    public function calendar(
        ...
        $manual = $this->scope(DB::table('financial_calendar_events'), $user)
            ...
            ->where(function (Builder $query) use ($to) {
                $query->whereDate('scheduled_for', '<=', $to->toDateString())
        ...
        if ($currency === 'UGX' && Schema::hasTable('credit_repayment_schedule_items')) {
            $loanEvents = DB::table('credit_repayment_schedule_items as schedule')
                ...
                ->whereBetween('schedule.due_date', [$from->toDateString(), $to->toDateString()])
            ...
            foreach ($loanEvents as $event) {
                $events[] = [
                    ...
                    'event_type' => 'loan',
                    'direction' => 'expense',
                    'amount_minor' => (int) $event->total_outstanding_minor,
                    'currency' => 'UGX',
                    ...
                    'source_reference' => (string) $event->loan_id,
                    'recurrence' => null,
                    'derived' => true,
                ];
            }
    ...
    public function suggestCategory(?string $description): string
    {
        ...
        $rules = [
            ...
            'Transport' => ['fuel', 'taxi', 'uber', 'boda', 'transport', 'bus'],
            'Rent' => ['rent', 'landlord'],
            'Utilities' => ['water', 'electricity', 'umeme', 'utility'],
            'School Fees' => ['school', 'tuition', 'fees'],
            'Health' => ['hospital', 'clinic', 'pharmacy', 'medical', 'health'],
            'Airtime/Data' => ['airtime', 'data bundle', 'internet', 'telecom'],
            'Business Stock' => ['stock', 'inventory', 'supplier'],
            'Family Support' => ['family', 'support'],
            'Loan Repayment' => ['loan', 'repayment'],
            'Savings' => ['save', 'savings'],
            'Insurance' => ['insurance', 'premium'],
            'Entertainment' => ['bar', 'cinema', 'entertainment'],
    ...
    private function debtOutstandingMinor(User $user, string $currency): int
    {
        ...
        if (Schema::hasTable('loan_schedules')) {
            return (int) round((float) DB::table('loan_schedules as schedule')
                ->join('loans', 'loans.id', '=', 'schedule.loan_id')
                ->where('loans.user_id', $user->id)
    ...
    private function savingsBalanceMinor(User $user): int
    {
        return SavingsGoal::query()
            ...
            ->get()
            ->sum(fn (SavingsGoal $goal) => $goal->confirmedBalanceMinor());
    }
    ...
    private function goals(User $user): array
    {
        return SavingsGoal::query()
            ...
            ->map(fn (SavingsGoal $goal) => [
                'reference' => $goal->goal_reference,
                'name' => $goal->name,
                'target_amount_minor' => (int) $goal->target_amount_minor,
                'balance_minor' => $goal->confirmedBalanceMinor(),
    ...
    private function activity(User $user, string $currency): array
    {
        ...
        $payments = MobileMoneyTransaction::query()
            ->where('user_id', $user->id)
            ->where('currency', $currency)
            ...
            ->map(fn (MobileMoneyTransaction $payment) => [
                ...
                'title' => $payment->direction === MobileMoneyTransaction::DIRECTION_COLLECTION
                    ? 'Money collected'
                    : 'Money sent',
    ...
    private function nextBestAction(
        ...
        if ($availableMinor === null) {
            return [
                'code' => 'record_balance',
                'title' => 'Add your current balance',
                'text' => 'Record a current cash, mobile-money or bank balance so OpFin can calculate safe-to-spend without guessing.',
                'href' => '/money',
                'action' => 'Add balance',
            ];
        }
        ...
        if ($committedMinor > $availableMinor) {
            return [
                'code' => 'cashflow_shortfall',
                'title' => 'Review an upcoming shortfall',
                'text' => 'Confirmed and scheduled obligations in the next 30 days are greater than your recorded available money.',
                'href' => '/calendar',
        ...
        if ($overBudget) {
            return [
                'code' => 'budget_overrun',
                'title' => 'Review '.$overBudget['category'].' spending',
                'text' => 'This category is above its monthly budget. Review recent entries before making new commitments.',
                'href' => '/money',
        ...
        if ($loanDue) {
            return [
                'code' => 'loan_due',
                'title' => 'Prepare for your next loan payment',
                'text' => 'A confirmed loan instalment is coming up. Keeping it funded protects your repayment record.',
                'href' => '/loans/account',
                'action' => 'View loan',
            ];
        }
        ...
        if ($goals !== []) {
            return [
                'code' => 'build_goal',
                'title' => 'Keep building '.$goals[0]['name'],
                'text' => 'Your recorded safe-to-spend is '.($safeToSpendMinor ?? 0).' minor units after upcoming commitments.',
                'href' => '/save',
                'action' => 'View goal',
            ];
        }
        ...
        return [
            ...
            'title' => 'Your next 30 days are funded',
            'text' => 'Review your budget and calendar before adding another recurring commitment.',
            'href' => '/money',
